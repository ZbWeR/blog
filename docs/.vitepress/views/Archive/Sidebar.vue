<template>
  <div class="lg:sticky lg:top-20">
    ...
    <div class="">
      ...
      <div class="mt-4 flex flex-col gap-4">
        <div
          ...
          class="group relative flex flex-1 items-center justify-between overflow-hidden rounded-lg bg-gradient-to-r p-5 text-white transition-all hover:cursor-pointer"
        ...
        >
          <div>
            ...
            <div
              class="grid grid-rows-[0fr] overflow-hidden transition-all duration-700 group-hover:grid-rows-[1fr]"
            >
              <p class="min-h-0 whitespace-pre-wrap">{{ category.desc }}</p>
            </div>
          </div>
          <p
            class="absolute right-2 top-6 rotate-12 scale-150 text-3xl opacity-20 blur-[2px] transition-all duration-700 group-hover:top-2 group-hover:rotate-0 group-hover:scale-100 group-hover:opacity-100 group-hover:blur-none"
          >
            {{ category.icon || '🔗' }}
          </p>
        </div>
    ...
    <!-- 随机一言 -->
    <div
      class="bg-text-card mt-4 rounded-lg bg-amber-100 px-2 py-2 shadow-md dark:bg-amber-950/80"
